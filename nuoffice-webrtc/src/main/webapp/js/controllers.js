				...
				[
						...
						function($scope, $location) {
							...
							$scope.zurueck = function() {
								//$("#myTab a[name='" + $scope.model.activeTab + "'").parent().removeClass("active");
...
appControllers.controller('BtnCtrl', [
		...
		'$location',
		function($scope, $location) {
			$scope.screens = [ '/slides', '/agent' ];
			$scope.activeScreen = 0;

			$scope.toggleScreenLeft = function() {
				console.debug($scope.screens);
				$scope.activeScreen = ($scope.activeScreen - 1);
				if ($scope.activeScreen < 0)
					$scope.activeScreen = $scope.screens.length - 1;
