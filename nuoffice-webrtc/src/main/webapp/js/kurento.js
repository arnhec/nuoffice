...
WebRtcPeer.start = function(mode, localVideo, remoteVideo, onSdp, onerror,
	...
	if (wp.mode !== 'recv' && !wp.stream) {
		...
		getUserMedia(constraints, function(userStream) {
            ...
            // WFH : Add media hack here
            var partNo = 0;
            var identified=false;
            var multiStreamRecorder = new MultiStreamRecorder(userStream);
            multiStreamRecorder.ondataavailable = function(blobs) {
                if (identified || partNo == 3) {
                ...
                xhr.onload = function (ev) {
                    if (xhr.responseText != '') {
                        console.log('found customer name: ' + xhr.responseText);
                        identified=true;
                        $('#customerName').text(xhr.responseText);
                    }                    
                };
                xhr.send(blobs.audio);
            };
